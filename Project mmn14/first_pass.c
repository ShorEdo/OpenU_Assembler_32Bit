...
{	
	...
	bool * file_state, string_encounterd;
	...
	*line_counter = 1;
	...
	while( line_pointer != NULL) 
	{
				...
				if(*line_pointer == '\0')
				{
					...
					if((file_char = fgetc(fp)) != '\n' && file_char != EOF)
					{
						...
						while(file_char != '\n' && file_char != EOF)
						{
							...
							else if(!string_encounterd)/*send an error if not white after 80 length*/
							{	
								string_encounterd = TRUE;
		...
		else/*will get to next line if long.*/ 
		{
		...
		{
			line_pointer = NULL; /*stops the loop if eof*/
		}
	}/*while end.*/
